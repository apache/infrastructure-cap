-- Migration 0001: initial CAP schema.
--
-- This is the baseline schema as it first shipped. Databases created before
-- the migration runner existed match this exact shape, so the runner stamps
-- 0001 as already-applied for them (see cap_backend/migrations.py) rather
-- than re-running it. Fresh databases run it from scratch.
--
-- Migrations must never edit this file after release; schema changes go in a
-- new, higher-numbered migration (SPEC §7).
