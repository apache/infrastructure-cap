"""Integration tests for /api and /list endpoints."""
...
async def test_api_is_public_and_valid_openapi(app):
    ...
    # /list and /api must appear in the served document.
    assert "/list" in body["paths"]
...
async def test_api_document_advertises_pydantic_schemas(app):
    ...
    # The full ResponseOption discriminated union must be reachable from /list
    # via ListResponse -> Question -> response_option.
    ...
    list_responses = body["paths"]["/list"]["get"]["responses"]
...
async def test_list_unauthenticated_returns_401_json(app):
    client = app.test_client()
    response = await client.get("/list", headers={"Accept": "application/json"})
...
async def test_list_unauthenticated_html_redirects_to_oauth(app):
    client = app.test_client()
    response = await client.get("/list", headers={"Accept": "text/html"})
...
async def test_api_response_is_cached_across_requests(app):
    client = app.test_client()
    r1 = await client.get("/api")
    r2 = await client.get("/api")
