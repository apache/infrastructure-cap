...
<div class="about-page">
  ...
  <section class="mb-4">
    ...
    <ul>
      <li>a <code>unanimous_approval</code> question is decided by whether
      any binding voter holds a veto</li>
      <li>a <code>majority_approval</code> question by
      counting binding votes against a minimum-three-votes floor</li>
      <li>a <code>simple_majority</code> question by counting binding votes with no floor</li>
      <li>a <code>lazy_consensus</code> question by whether any objection (binding or
  ...
  <ol>
    ...
    <li>
      ...
      <code>GET /api/token</code>. CAP assigns the <code>request_id</code>
      and <code>question_id</code> server-side and returns them in the
      <code>201</code> response (the request body must <em>not</em> carry
      them); the calling service stores those identifiers so it can
      correlate the future verdict back to its own state.
    </li>
    <li>
      The service either subscribes to the CAP pubsub topic for that
      project, or polls
      <code>GET /api/resolution/&#123;question_id&#125;</code> at a slow
      cadence. That permalink endpoint encodes the verdict in its status
      code: <code>204</code> while the question is still open,
      <code>200</code> once it is resolved and approved, and
      <code>424</code> for any other terminal outcome (vetoed,
      insufficient votes, or withdrawn). The pubsub path is preferred; it
      delivers the <code>question.resolved</code> event as soon as the
      tally has been recorded, so the service can react with no polling
      lag.
    </li>
    <li>
